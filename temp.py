...
def main() -> int:
    ...
    parser.add_argument("url", nargs="?", help="YouTube URL or video ID. If omitted, the script asks at runtime.")
    ...
    parser.add_argument("--ffmpeg-location", default="", help="Path to ffmpeg binary or containing folder. Also reads FFMPEG_LOCATION env var.")
    parser.add_argument("--cookies-from-browser", default="", help="Browser for yt-dlp cookies, e.g. firefox, chrome. Also reads YTDLP_COOKIES_FROM_BROWSER env var.")
    parser.add_argument("--cookies", default="", help="Path to cookies.txt for yt-dlp. Also reads YTDLP_COOKIES env var.")
    parser.add_argument("--sleep-requests", default="1", help="yt-dlp --sleep-requests value.")
    ...
    parser.add_argument("--ytdlp-retries", default="10", help="yt-dlp retry count.")
    ...
    FFMPEG_LOCATION = args.ffmpeg_location or FFMPEG_LOCATION
    YTDLP_COOKIES_FROM_BROWSER = args.cookies_from_browser or YTDLP_COOKIES_FROM_BROWSER
    YTDLP_COOKIES = args.cookies or YTDLP_COOKIES
    YTDLP_SLEEP_REQUESTS = args.sleep_requests or YTDLP_SLEEP_REQUESTS
    YTDLP_SLEEP_INTERVAL = args.sleep_interval or YTDLP_SLEEP_INTERVAL
    YTDLP_MAX_SLEEP_INTERVAL = args.max_sleep_interval or YTDLP_MAX_SLEEP_INTERVAL
    YTDLP_RETRIES = args.ytdlp_retries or YTDLP_RETRIES
    ...
    if not which("ffmpeg"):
        log("ffmpeg not found. Frame extraction will be skipped. Install ffmpeg or set FFMPEG_LOCATION.")
    ...
    if not skill_md:
        log("SKILL.md generation failed. Check OPENAI_API_KEY and API connectivity.")
        return 1
